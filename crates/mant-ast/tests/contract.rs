...
const MINIMAL_QUERY: &str = include_str!("../../../tests/contracts/minimal-query-v7.json");
...
fn shared_query_fixture_round_trips_without_shape_changes() {
    ...
    assert_eq!(query.schema, QuerySchema::V7);
...
fn unknown_query_schema_is_rejected() {
    let incompatible = MINIMAL_QUERY.replace("mant.query/v7", "mant.query/v1");
...
fn native_query_request_covers_every_projection_and_rejects_unknown_fields() {
    let request: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"printf","section":"3"},"view":{"kind":"full"}}"#,
    )
    ...
    let outline: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"outline","detail":"entries"}}"#,
    )
    ...
    let excerpt: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"excerpt","nodes":["acls"]}}"#,
    )
    ...
    let explain: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"explain","entry":"--exclude"}}"#,
    )
    ...
    let search: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"search","pattern":"--acls","syntax":"literal","case":"insensitive","scope":"visible","word":false,"contextLines":2,"limit":20,"offset":0}}"#,
    )
    ...
    let search_defaults: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"search","pattern":"acls"}}"#,
    )
    ...
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"ls"},"view":{"kind":"full"},"mode":"html"}"#,
    )
    ...
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"ls"},"view":{"kind":"full","future":true}}"#,
    )
...
#[test]
fn request_v7_rejects_the_obsolete_options_outline_detail() {
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"tar"},"view":{"kind":"outline","detail":"options"}}"#,
    )
    .expect_err("request v7 uses the canonical entries spelling");
    assert!(error.to_string().contains("unknown variant `options`"));
}

#[test]
fn request_v7_selects_one_configured_source_without_accepting_v4() {
    let selected: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v7","input":{"kind":"document","name":"printf","source":"team"},"view":{"kind":"full"}}"#,
    )
