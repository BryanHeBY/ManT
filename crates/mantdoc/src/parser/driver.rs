use super::{
    ...
    emit_mdoc_new_sentence_line_warnings, emit_outside_macro_argument_escapes,
    emit_trailing_whitespace, emit_unterminated_quoted_argument,
    ...
    execute_transparent_request, expand_copy_mode_definition, expand_declared_character_escapes,
...
impl<R: SourceResolver + ?Sized> SourceFrame<'_, '_, '_, R> {
    ...
    pub(super) fn run(self) -> ParseState {
        let Self {
            source,
            source_id,
            include_depth,
        ...
        let builder = &mut *core.builder;
        let environment = &mut *core.environment;
        let active_sources = &mut *core.active_sources;
        let resolver = &mut *core.resolver;
        let ParseState {
            mut diagnostics,
                ...
                SourceEvent::Control(ControlEvent {
                            ...
                            let virtual_scope_opening_position = scope_opening_column
                                ...
                                .and_then(|_| {
                                    ...
                                    let control_position =
                                        builder.source_position(&control_span)?;
                                    let prefix_width =
                                    ...
                                    Some(SourcePosition {
                                        ...
                                        column: control_position
                                            .column
                                            .saturating_add(u32::try_from(column).expect(
                            ...
                            lex_condition_arguments(arguments, scanner.escape_character(), limits)
                    {
                        ...
                        let body_template = if let Some(trimmed) = inline_scope_body {
                            let mut normalized =
                                Vec::with_capacity(trimmed.len().saturating_add(2));
                        ...
                        let body_source_start =
                            inline_scope_source_start.unwrap_or(body_source_start);
                        let escape = scanner.escape_character();
                        let scope_remainder = scope_opener_remainder(&body_template, escape);
                        let scope_requested = scope_remainder.is_some();
                        ...
                        {
                            let blank_start = scope_remainder_source_start(
                                &body_template,
                                body_source_start,
                                escape,
                            );
                    ...
                    if name == b"so" {
                        ...
                        let mut included_core =
                            ParserCore::new(config, builder, environment, active_sources, resolver);
                        let outcome = SourceFrame::new(
                            ...
                            lex_arguments(arguments, scanner.escape_character(), limits)
                    {
                        ...
                        if !indirect && !name_terminates_at_tab && arguments.get(2).is_some() {
                            let ignored_after_tab = arguments.get(1).is_some_and(|terminator| {
                                terminator.separator_after == Some(b'\t')
                            });
                            push_diagnostic(
                                &mut diagnostics,
                                limits,
                    ...
                    if name == b"ig" {
                        let arguments = match lex_arguments(
                            arguments,
                            scanner.escape_character(),
                            limits,
                        ) {
                        ...
                        && (builder.macro_set() == MacroSet::Mdoc || config.syntax == Syntax::Mdoc)
                    {
                        match lex_arguments(arguments, scanner.escape_character(), limits) {
                            ...
                            Ok(arguments) => {
                                ...
                                builder
                                    .operating_system(visible_bytes(&join_arguments(&arguments)));
                            }
                    ...
                    if !builtin_package_macro && environment.is_conditionally_unknown_macro(name) {
                        ...
                        if let Err(error) =
                            environment.define_macro(name, Vec::new(), false, limits)
                        {
                            truncated = true;
                            push_diagnostic(
                                &mut diagnostics,
                            ...
                            if let Some((request, raw_arguments)) = split_macro_control(
                                ...
                                if matches!(request, b"if" | b"ie" | b"el") {
                                    ...
                                    let (condition, body_start, predicate_width) = match request {
                                        ...
                                        b"if" | b"ie" => {
                                            if request == b"ie"
                                                && (condition_arguments.is_empty()
                                                    || condition_arguments.first().is_some_and(
                                                        |argument| argument.bytes == b"!",
                                                    ))
                                            {
                                                macro_conditionals
                                ...
                                if matches!(request, b"de" | b"de1" | b"am" | b"dei" | b"ami") {
                                    ...
                                    if matches!(request, b"de" | b"de1") {
                                        while pending.last().is_some_and(
                                            |(_, _, depth, _, _, _)| *depth == macro_depth,
                                        ) {
                                    ...
                                    while !terminated
                                        && let Some(body_line) = scanner.next_raw_line()
                                    {
                                        if is_definition_terminator(
                                            body_line.bytes,
                                            definition_control,
                                    ...
                                    )
                                {
                                    ...
                                    let scope_cursor =
                                            ...
                                            .saturating_add(u32::try_from(body_line.len()).expect(
                                        ...
                                        environment.macro_definition(request).cloned()
                                {
                                    if macro_definition_directly_invokes(
                                        ...
                                        pending
                                            .retain(|(_, _, depth, _, _, _)| *depth < macro_depth);
                                        continue;
                                    }
                                ...
                                let generated_control_position = builder
                                    ...
                                    .map(|position| SourcePosition {
                                        line: position.line,
                                        column: macro_origin.saturating_add(
                                            macro_body_control_column(
                                                &body_line,
                                                scanner.control_character(),
                                            ),
                                        ),
                                ...
                                if !raw_arguments.is_empty() {
                                    ...
                                    let macro_body_separator_widths =
                                        lex_arguments(raw_arguments, escape, limits)
                                            ...
                                            .unwrap_or_default();
                                    let Ok(arguments) = lex_arguments(&bytes, escape, limits)
                                    else {
                                        truncated = true;
                                        push_diagnostic(
                                            &mut diagnostics,
                                    ...
                                    let all_arguments_atom_width = u32::try_from(
                                        raw_arguments.len(),
                                    )
                                    ...
                                    for argument in arguments {
                                        ...
                                        if append_text_node(
                                            ...
                                            {
                                                let _ = builder
                                                    .set_node_logical_start(argument, position);
                                            }
                                            if all_arguments_expansion {
                                                next_generated_argument_position =
                                                    next_generated_argument_position.map(
                    ...
                    let character_request =
                        matches!(dispatched_package_macro, b"cc" | b"c2" | b"ec");
                    ...
                    let renamed_package_argument_position = renamed_package_macro
                        .then(|| {
                    ...
                    let parsed_arguments = match lex_arguments(arguments, argument_escape, limits) {
                        ...
                        Err(ArgumentIssue::UnterminatedQuote) => {
                            ...
                            // element.
                            recover_unterminated_quoted_arguments(
                                arguments,
                                argument_escape,
                                limits,
                            )
                        ...
                        Ok(mut arguments) => {
                            ...
                            for argument in arguments {
                                ...
                                let expanded = if character_request {
                                    ...
                                    // escaped spelling.
                                    let reparsed =
                                        (builder.macro_set() != MacroSet::None).then(|| {
                                            copy_mode_reparse(
                                                &argument.bytes,
                                                scanner.escape_character(),
                                            )
                                ...
                                let expansion_width_delta =
                                    if preserves_copy_mode_e_width
                                        && text
                                            .as_bytes()
                                            .windows(2)
                                ...
                                if append_text_node(
                                    ...
                                    if let Some(argument_node) = builder
                                        ...
                                        .copied()
                                    {
                                        let _ = builder.set_node_separator_after(
                                            argument_node,
                                            separator_after,
                                        );
                                        ...
                                        let _ = builder.set_node_separator_width(
                                            argument_node,
                                            separator_width,
                                        );
