...
fn slash_option_aliases_preserve_forms_and_query_identity() {
    for declaration in ["", "<!-- mant:entries role=option case=sensitive -->\n"] {
        for form in ["-h/--help", "-h, --help", "-h|--help"] {
            ...
            assert_eq!(index.root()[0].aliases, ["-h", "--help"], "{source}");
            ...
            let OutlineNode::DocumentEntry { aliases, forms, .. } = &children[0] else {
                panic!("{children:?}")
            };
            assert_eq!(aliases, &["-h", "--help"]);
            assert_eq!(forms, &[form]);
            let short = crate::select_explanation(&query, "-h").unwrap();
            let long = crate::select_explanation(&query, "--help").unwrap();
    ...
    for (declaration, form, names) in [
        ...
        .unwrap();
        let index = mant_ir::SemanticIndex::build(query.document.as_ref().unwrap());
        assert_eq!(index.root()[0].aliases, names, "{form}");
        assert_eq!(index.root()[0].forms, [form]);
        assert!(crate::select_explanation(&query, "--help").is_err());
    }
}
