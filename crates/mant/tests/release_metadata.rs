...
fn workspace_crates_own_their_versions_and_use_explicit_caret_dependencies() {
    ...
    for (name, manifest) in [
        ...
        ("mant-query", include_str!("../../mant-query/Cargo.toml")),
        ("mant-render", include_str!("../../mant-render/Cargo.toml")),
...
fn selected_crates_are_published_in_dependency_order_at_their_own_versions() {
    ...
    assert!(publish.contains(
        "CRATE_TAG_PACKAGES=(mant-ir mant-protocol libmandoc-rs mant-sources mant-codec mant-loader mant-query mant-render mant-engine mant-ui)"
    ));
    assert!(include_str!("../../../.github/workflows/release.yml").contains(
        "packages=(mant-ir mant-protocol libmandoc-rs mant-sources mant-codec mant-loader mant-query mant-render mant-engine mant-ui)"
    ));
...
fn packaged_and_windows_checks_include_extracted_package_test_surfaces() {
    ...
    for package in ["mant-engine", "mant-ui", "mant"] {
        ...
        assert!(dependency_line.contains("mant-query"), "{dependency_line}");
        assert!(dependency_line.contains("mant-render"), "{dependency_line}");
    }
...
fn publication_tags_are_validated_before_registry_authentication() {
    ...
    let codec_version = include_str!("../../mant-codec/Cargo.toml")
        .lines()
        .find_map(|line| line.strip_prefix("version = \"")?.strip_suffix('"'))
    ...
    let query_version = include_str!("../../mant-query/Cargo.toml")
        .lines()
        .find_map(|line| line.strip_prefix("version = \"")?.strip_suffix('"'))
        .expect("mant-query version");
    let render_version = include_str!("../../mant-render/Cargo.toml")
        .lines()
        .find_map(|line| line.strip_prefix("version = \"")?.strip_suffix('"'))
        .expect("mant-render version");
    ...
    for (tag, package) in [
        ...
        (format!("mant-query-v{query_version}"), Some("mant-query")),
        (
            format!("mant-render-v{render_version}"),
            Some("mant-render"),
        ),
    ] {
