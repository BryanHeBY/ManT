...
use super::{TextPresentation, TextRole, sanitize_terminal_text};
use mant_protocol::{
...
/// This readable label is not a reusable URI. Use [`mant_ir::LinkTarget::to_uri`] for
...
mod tests {
    ...
    fn copied_manual_targets_keep_the_native_scheme_without_display_annotations() {
        for (section, expected) in [(None, "man:printf"), (Some("3"), "man:printf(3)")] {
            ...
            assert_eq!(target.to_uri().as_deref(), Some(expected));
        }
    }
