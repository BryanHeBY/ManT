...
use mant_protocol::DocumentCatalog;
...
mod roles;
pub use roles::{EntryTone, entry_tone};
mod bindings;
...
#[must_use]
pub fn render_explanation_status(result: &mant_protocol::QueryExplanation) -> String {
    ...
    if result.truncation != mant_protocol::ExplanationTruncation::default() {
        write!(
            text,
...
#[must_use]
pub fn render_evidence_heading(evidence: &mant_protocol::ExplanationEvidence) -> String {
...
pub fn render_catalog_coverage_text(catalog: &DocumentCatalog) -> Option<String> {
    ...
    match catalog.query.kind {
        Some(mant_protocol::CatalogDocumentKind::Manual) => Some("no manuals indexed".to_owned()),
        Some(mant_protocol::CatalogDocumentKind::Markdown) => {
