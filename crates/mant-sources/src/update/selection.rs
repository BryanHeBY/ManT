...
pub(in crate::update) fn install_selected_documents(
    ...
    InstallPlan::select(checkout, source)?.stage(staging)
}

/// Logical collisions are resolved before any selected document is written.
struct InstallPlan {
    selected: BTreeMap<String, (String, u8, PathBuf, PathBuf)>,
}
impl InstallPlan {
    fn select(checkout: &Path, source: &ConfiguredSource) -> Result<Self, String> {
    ...
    fn stage(self, staging: &Path) -> Result<usize, String> {
        let mut installed_bytes = 0_u64;
        for (_, _, relative, path) in self.selected.values() {
