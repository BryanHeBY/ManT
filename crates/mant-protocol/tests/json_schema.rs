...
fn response_schemas_follow_the_serialized_wire_shapes() {
    for (schema, marker) in [
        (query_bundle_json_schema(), "mant.query/v0.9"),
        (query_outline_json_schema(), "mant.outline/v0.9"),
        (query_excerpt_json_schema(), "mant.excerpt/v0.9"),
        (query_search_json_schema(), "mant.search/v0.9"),
        (document_catalog_json_schema(), "mant.catalog/v0.9"),
    ] {
        let encoded = serde_json::to_string(&schema).expect("schema JSON");
    ...
    assert!(encoded_query.contains("mant.document/v0.9"));
