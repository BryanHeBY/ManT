...
const V0_8_SNAPSHOT: &str = include_str!("../../../tests/contracts/protocol-schemas-v0.8.json");
...
#[test]
fn v0_8_structural_schemas_change_only_with_an_explicit_protocol_version() {
    assert_eq!(NATIVE_API_VERSION, "0.8");

    let mut expected: Value = serde_json::from_str(V0_8_SNAPSHOT).expect("v0.8 schema snapshot");
    ...
    assert_eq!(
        actual, expected,
        "the v0.8 structural contract changed; restore compatibility or advance every affected schema discriminator before regenerating the snapshot"
    );
}

#[test]
fn v0_8_catalog_exposes_only_logical_document_identities() {
