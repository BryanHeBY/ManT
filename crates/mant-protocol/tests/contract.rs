...
const MINIMAL_QUERY: &str = include_str!("../../../tests/contracts/minimal-query-v0.9.json");
...
fn shared_query_fixture_round_trips_without_shape_changes() {
    ...
    assert_eq!(query.schema, QuerySchema::V0Dot9);
...
fn unknown_query_schema_is_rejected() {
    let incompatible = MINIMAL_QUERY.replace("mant.query/v0.9", "mant.query/v1");
...
fn native_query_request_covers_every_projection_and_rejects_unknown_fields() {
    let request: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"printf","manualSection":"3"},"view":{"kind":"full"}}"#,
    )
    ...
    let outline: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"outline","detail":"entries"}}"#,
    )
    ...
    let excerpt: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"excerpt","selectors":["acls"]}}"#,
    )
    ...
    let explain: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"explain","entry":"--exclude"}}"#,
    )
    ...
    let search: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"search","pattern":"--acls","syntax":"literal","case":"insensitive","scope":"visible","word":false,"contextLines":2,"limit":20,"offset":0}}"#,
    )
    ...
    let search_defaults: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"search","pattern":"acls"}}"#,
    )
    ...
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"ls"},"view":{"kind":"full"},"mode":"html"}"#,
    )
    ...
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"ls"},"view":{"kind":"full","future":true}}"#,
    )
...
#[test]
fn request_v0_9_rejects_the_obsolete_options_outline_detail() {
    let error = serde_json::from_str::<QueryRequest>(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"tar"},"view":{"kind":"outline","detail":"options"}}"#,
    )
    .expect_err("request v0.9 uses the canonical entries spelling");
...
fn scope_request_is_closed_bounded_and_keeps_single_document_views_separate() {
    let request: ScopeQueryRequest = serde_json::from_str(
        r#"{"schema":"mant.scope-request/v0.9","scope":{"documents":[{"selector":"git"},{"selector":"manual/1/git-add"}],"traversal":{"followLinks":true,"maxDepth":3,"maxDocuments":12}},"view":{"kind":"search","pattern":"index"}}"#,
    )
    .expect("valid scope request");
    assert_eq!(request.schema, ScopeRequestSchema::V0Dot9);
    ...
    let error = serde_json::from_str::<ScopeQueryRequest>(
        r#"{"schema":"mant.scope-request/v0.9","scope":{"documents":[{"selector":"git"}]},"view":{"kind":"outline"}}"#,
    )
    .expect_err("outline remains a single-document request");
    assert!(error.to_string().contains("unknown variant"));

    let error = serde_json::from_str::<ScopeQueryRequest>(
        r#"{"schema":"mant.scope-request/v0.9","scope":{"documents":[{"selector":"git"}],"future":true},"view":{"kind":"explain","entry":"clone"}}"#,
    )
...
#[test]
fn request_v0_9_selects_one_configured_source_without_accepting_v4() {
    let selected: QueryRequest = serde_json::from_str(
        r#"{"schema":"mant.request/v0.9","input":{"kind":"document","selector":"printf","source":"team"},"view":{"kind":"full"}}"#,
    )
...
fn an_empty_catalog_is_protocol_owned_and_versioned() {
    ...
    assert_eq!(catalog.schema, mant_protocol::CatalogSchema::V0Dot9);
