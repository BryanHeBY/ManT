//! Tests for the `gawk(1)` fixture — operator precedence table inline-term
//! decisions.
...
fn keeps_section_topology() {
    common::assert_section_topology("fedora44/gawk", fedora44_manual("gawk"), GAWK_SECTIONS);
}
...
fn man_format_renders_operator_table_tight() {
    let output = render_query_man(&fedora44_manual_query("gawk"));
