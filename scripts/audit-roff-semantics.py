...
def valid_entry(value: object) -> bool:
    ...
    return (
        ...
        and all(
            ...
            for field in ("names", "forms", "targets")
        )
    ...
    *, exact_roots=None, batch_runner=None,
    ...
    for offset in range(0, len(items), 256):
        ...
        responses = (batch_runner(profiler, batch, timeout) if batch_runner else
                     run_jsonl_profile_batch(profiler, batch, timeout, "semantic"))
        for request_id, response in responses.items():
            ...
            retained_presentation_ordinals = response.get("retainedPresentationOrdinals")
            ...
            valid = (
                ...
                and isinstance(retained_presentation_ordinals, list)
                and all(valid_definition_candidate(item) for item in retained_presentation_ordinals)
