...
# The product's roff escape decoder deliberately preserves this narrow group
# of documented groff caron spellings even though pinned CVS mandoc does not
# provide them in ``chars.c``. Keep the audit projection equally narrow: this
# is not a general roff interpreter or a promise that arbitrary groff
# character names survive lowering. The mapping mirrors the source-preserving
# compatibility fact in ``mant-codec/src/mandoc/roff_escape/glyphs.rs``.
_GROFF_NAMED_GLYPHS = {
    "vC": "Č",
    "vc": "č",
    "vS": "Š",
...
_GROFF_NAMED_GLYPH_IN_WORD = re.compile(
    ...
    r"\\(?:(?P<compact>\()(?P<compact_name>v[CcSsZz])|\[(?P<bracket_name>v[CcSsZz])\])"
...
def _literal_groff_named_glyph_words(source: str) -> Counter[tuple[str, str]] | None:
    ...
    The groff spellings listed above are not present in CVS ``chars.c`` (so
    its terminal formatter drops them), while groff accepts them and ManT
    deliberately preserves their authored Unicode. A source inventory is
    useful only when no request or escape can mutate the text stream, and only
    when both the fallback and decoded spellings are complete whole-word
    inventories. This prevents an unrelated literal fallback word from
    borrowing the source proof.
    """
    ...
    for raw in source.splitlines():
        ...
        for match in _GROFF_NAMED_GLYPH_IN_WORD.finditer(raw):
            name = match["compact_name"] or match["bracket_name"]
            glyph = _GROFF_NAMED_GLYPHS[name]
...
def _literal_eqn_relation_spellings(source: str) -> Counter[tuple[str, str]] | None:
    """Return source-bound simple eqn relation spellings, or decline execution.

    CVS ``eqn_term.c`` emits positional markers directly against their bases
    (``log_2`` and ``a/b``), whereas ManT retains the same parsed relations
    ...
    in_block = False
    saw_equations = False
    pending_delimiters: tuple[str, str] | None = None
    active_delimiters: tuple[str, str] | None = None

    def collect(expressions: list[str]) -> bool:
        for expression in expressions:
            if _DYNAMIC_ESCAPE.search(expression):
                return False
            ...
            for match in _EQN_LITERAL_BRACED_FROM.finditer(expression):
                ...
                # renderer-specific: CVS exposes only `base_` while ManT
                # retains a separated marker before the complete operand.
    ...
    for raw in source.splitlines():
        if _EQN_OPEN.fullmatch(raw):
            ...
            in_block = True
            saw_equations = True
            pending_delimiters = None
            continue
        if _EQN_CLOSE.fullmatch(raw):
            if not in_block:
                return None
            in_block = False
            active_delimiters = pending_delimiters
            continue
        if in_block:
            if raw.endswith("\\"):
                return None
            delimiter = raw.strip().split()
            if delimiter[:1] == ["delim"]:
                if len(delimiter) != 2:
                    return None
                if delimiter[1] == "off":
                    pending_delimiters = None
                elif len(delimiter[1]) == 2:
                    pending_delimiters = (delimiter[1][0], delimiter[1][1])
                else:
                    return None
                continue
            if not collect([raw]):
                return None
            continue
        if active_delimiters is None:
            continue
        opening, closing = active_delimiters
        # Delimiters are literal one-character operands from the balanced
        # configuration line above; regex escaping keeps punctuation syntax
        # from broadening this source inspection.
        expressions = re.findall(
            re.escape(opening) + r"([^\n" + re.escape(closing) + r"]*)" + re.escape(closing),
            raw,
        )
        if expressions and not collect(expressions):
            return None
    if in_block:
        return None
    return spellings if saw_equations and spellings else None
...
def _source_consistent_compatibility_projection(reference: str, mant: str,
    ...
    # CVS mandoc's character table intentionally omits these documented groff
    # caron spellings, while groff and ManT preserve their authored Unicode.
    # Exact inventories on all three sides make this a reference-compatibility
    # distinction rather than an excuse for arbitrary Unicode substitutions.
    ...
    eqn_relations = _literal_eqn_relation_spellings(source)
    # CVS eqn_term.c writes positional operators under TERMP_NOSPACE while
