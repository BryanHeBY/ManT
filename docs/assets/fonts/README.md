...
The reader screenshot bundles the static JetBrains Mono 2.304 Regular,
Medium, Bold, Italic, and Bold Italic TrueType files from the
...
directory and fixes grayscale antialiasing and hinting for the capture. The
Fontconfig rules map ordinary terminal text to Medium so its strokes remain
clear when the screenshot is scaled down in the README, while xterm maps
ManT's semantic bold text to the distinct Bold face.
