...
Cell text passes through the same roff escape decoder as ordinary prose. Source-backed `T{`/`T}` blocks follow the pinned `mandoc` tbl dispatch boundary rather than running a second man/mdoc parser: lexical `\"` and `\#` comments and the active `.ec`/`.eo` escape state apply first; native roff requests are executed or suppressed by the parser; and an unknown or high-level man/mdoc control line contributes only its raw operands to the cell. Thus `.BR A / B .` contributes `A / B .`, `.Sm off` contributes `off`, and `.Fl Fl help` contributes `Fl help`; they do not create links, semantic entries, font scopes, or spacing state inside the table.

Recovery is transactional at the cell boundary. Native cell text is authoritative when present: source text can replace it only when normalized visible text is identical, preventing a nearby text block or unevaluated expansion from changing a cell. When libmandoc exposes an empty cell, ManT may retain the bounded raw source payload so formatter-specific spelling remains visible. This policy preserves content without inventing a second parser or claiming table-local macro semantics.
