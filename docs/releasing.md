...
1. Version the eleven crates independently in their own `Cargo.toml` files.
...
`mant-sources`, `mant-codec`, `mant-loader`, `mant-query`, `mant-render`, `mant-engine`, and `mant-ui`. The tag version must exactly match
